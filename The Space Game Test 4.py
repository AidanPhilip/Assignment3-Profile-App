...
class Tower:
    ...
    def _checkCollision(self):
        ...
        allTowers = allOpenTowers + allClosedTowers + allOpenNodes + allClosedNodes
    ...
    def drawConnections(self):
        for connection in self.connections:
            pygame.draw.line(SCREEN, RELAY_BLUE, self.position, connection.position,2)

    def drawGhost(self):
        global collision
        pygame.draw.circle(SCREEN, ENERGY_CONNECTION_GREEN, self.position, self.connectionRange, 1) ## ENERGY CONNECTION RANGE
        if(self._checkCollision() == True):
            collision = True
            pygame.draw.circle(SCREEN, ERROR_RED, self.position, self.size, 0)
        else:
            collision = False
            pygame.draw.circle(SCREEN, HIGHLIGHT_GREEN, self.position, self.size, 0)
...
class WeaponTower(Tower):
    ...
    def drawGhost(self):
        super().drawGhost()
        pygame.draw.circle(SCREEN, BLACK, self.position, self.weaponRange, 1)
...
def handleGhost(position):
    towerObjectBeingPlaced.position = position
    towerConnections = towerObjectBeingPlaced.findNearestOpenTowers()
    for connection in towerConnections:
        pygame.draw.line(SCREEN, GHOST_BLUE, position, connection.position,2)
...
BLASER_BUTTON = TowerButton(1,BasicLaserTower(None))
MISSILE_LAUNCHER_BUTTON = TowerButton(2,MissileTower(None))
ENERGY_NODE_BUTTON = TowerButton(3,EnergyNode(None))
MINERAL_MINER_BUTTON = TowerButton(4,MineralMiner(None))
SOLAR_STATION_BUTTON = TowerButton(5,SolarStation(None))
